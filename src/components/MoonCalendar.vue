    ...
    <div class="calendar-days">
      ...
      >
        <div class="d-flex flex-column align-center">
          ...
          <div v-if="day.isCurrentMonth" class="day-icon d-flex flex-column align-center">
            <v-icon v-if="day.phaseIcon" :icon="day.phaseIcon" size="x-small" class="phase-icon"></v-icon>
...
const getPhaseIcon = (date: Date, prevDate?: Date, nextDate?: Date): string | null => {
  try {
...
const calendarDays = computed<CalendarDay[]>(() => {
  ...
  for (let date = 1; date <= daysInMonth; date++) {
    ...
    const isSelected = fullDate.toDateString() === (moonStore.currentDate || today).toDateString();
  ...
  for (let i = 0; i < currentMonthDays.length; i++) {
    ...
    const nextDate = i < currentMonthDays.length - 1 ? currentMonthDays[i + 1].fullDate : undefined;
...
const selectDate = (day: any) => {
  if (day.isCurrentMonth) {
    moonStore.setCurrentDate(day.fullDate);
  }
};
</script>
